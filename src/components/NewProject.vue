...
export default {
  ...
  setup () {
    ...
    const formattedTimestamp = date.formatDate(timeStamp, 'YYYY-MM-DDTHH:mm:ss.SSSZ')
    ...
    function saveLocally (f) {
      try {
        ...
        window.resolveLocalFileSystemURL(cordova.file.dataDirectory, function (dirEntry) {
          ...
          this.logs.push('> createFile is called, dirEntry.name: ' + dirEntry.name)
          this.$q.notify({
            icon: 'check_circle',
            color: 'positive',
      ...
      try {
        ...
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
          ...
          fs.root.getFile(f.name, { create: true, exclusive: false }, function (fileEntry) {
            ...
            this.logs.push('> requestFileSystem:OnErrorCreateFile callback error' + err)
          })
          // eslint-disable-next-line no-undef
        }, (err) => {
          this.logs.push('> requestFileSystem:OnErrorLoadFs callback error' + err)
        })
      } catch (e) {
        this.logs.push('> Error at outer catch: ' + e)
        this.$q.notify({
  ...
  methods: {
    ...
    uploadFile () {
      ...
      const storageRef = this.$fbstorageref(this.$fbstorage, `files/${files[0].name.split('.')[0]}.${files[0].name.split('.')[1]}`)
      ...
      uploadTask.on('state_changed',
        ...
        () => {
          ...
          this.$getdownloadurl(uploadTask.snapshot.ref).then((downloadURL) => {
            ...
            this.projectAvatar = `${files[0].name.split('.')[0]}.${files[0].name.split('.')[1]}`
          })
        }
      )
    },
